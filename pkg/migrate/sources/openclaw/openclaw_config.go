...
func (c *OpenClawConfig) ConvertToOperator(sourceHome string) (*OperatorConfig, []string, error) {
	cfg := &OperatorConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via operator CLI",
				len(c.Skills.Entries),
			),
		)
	}
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - Operator uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in Operator - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in Operator - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - Operator uses per-agent sessions by default")
	}
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No Operator adapter available")
	}
	if c.Channels.Matrix != nil {
		*warnings = append(*warnings, "Channel 'matrix': No Operator adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No Operator adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No Operator adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No Operator adapter available - consider iMessage instead",
		)
	}
...
func (c *OperatorConfig) ToStandardConfig() *config.Config {
