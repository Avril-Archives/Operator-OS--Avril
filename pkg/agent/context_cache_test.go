...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "operator-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleSystemMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sys := msgs[0].Content
			if !strings.Contains(sys, "operator") {
...
// (~/.operator/skills) invalidates the cached system prompt.
func TestGlobalSkillFileContentChange(t *testing.T) {
	...
	globalSkillPath := filepath.Join(tmpHome, ".operator", "skills", "global-skill", "SKILL.md")
...
func TestBuiltinSkillFileContentChange(t *testing.T) {
	...
	t.Setenv("OPERATOR_BUILTIN_SKILLS", builtinRoot)
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "operator-bench-*")
	defer os.RemoveAll(tmpDir)
