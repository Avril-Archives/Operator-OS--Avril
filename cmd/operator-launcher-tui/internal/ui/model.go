...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					operatorconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model operatorconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []operatorconfig.ModelConfig) {
...
func isModelValid(model operatorconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *operatorconfig.ModelConfig) {
	if model == nil {
		return
	}
