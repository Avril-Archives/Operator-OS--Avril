...
INTERNAL=github.com/operatoronline/Operator-OS/cmd/operator/internal
...
WORKSPACE_DIR?=$(OPERATOR_HOME)/workspace
...
## build: Build the operator binary for current platform
build: generate
...
## build-all: Build operator for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install operator to system and copy builtin skills
install: build
...
## uninstall: Remove operator from system
uninstall:
...
## uninstall-all: Remove operator and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(OPERATOR_HOME)
	@echo "Removed workspace: $(OPERATOR_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build operator-agent operator-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build operator-agent operator-gateway
...
## docker-run: Run operator gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run operator gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run operator agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm operator-agent

## docker-run-agent-full: Run operator agent in Docker (interactive, full-featured)
...
docker-clean:
	...
	docker rmi operator:latest operator:full 2>/dev/null || true
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.operator/workspace)"
